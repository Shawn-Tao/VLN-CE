...
class Json_Templater:
    def __init__(self):
        ...
        self.template_header_str = f"You are a robot programmed for navigation tasks. You have given "
        ...
        self.template_last_action_str = f"The sequential action decisions you made earlier are: \"<action>\"."
        # self.template_last_action_str = f"Your last action was: \" <action> \"."
        # self.template_task_str = f"Your assigned task is: \"<task>\". Analyze the above information and decide your next action: whether to move forward a specific distance, turn left or right by a specific angle, or stop if task is completed."
        self.template_task_str = f"Your assigned task is: \"<task>\". Analyze the above information and decide your next action: whether to move forward a specific distance, turn left or right by a specific angle, or determine that the task is complete and stop."
            ...
            for step in range(len(merged_actions)):
                ...
                if(step == 0):
                    ...
                    if(step < self.max_historical_image_num):    
                        ...
                        for i in range(step):
                            image_list.append(f"{self.reduced_image_path_prefix}/{episode_id}/{episode_id}_{merged_indices[step-i-1][0]}.jpg")
                    else:
                ...
                #     history_str = self.temlpate_history_str.replace("<history_image>",historical_image_str)
                #     for i in range(step):
                #         image_list.append(f"{self.path_prefix}/{episode_id}/{episode_id}_{merged_indices[step-i-1][0]}.jpg")
                #     image_list.reverse()    
                #     image_list.append(f"{self.path_prefix}/{episode_id}/{episode_id}_{merged_indices[step][0]}.jpg")
                # else:
                #     historical_image_str = ""
                ...
                if len(action_squence) > 0:
                    action_squence_str = ' '.join([f'{self.action_to_str(action)},' for action in action_squence])
                    # 将动作序号转换成字符串
                    history_action_str = self.template_last_action_str.replace("<action>", action_squence_str[:-1])
                
                json_user = {
                    "messages": [{
                        "content": f"{header_str}{history_str}{current_str}{history_action_str}{task_str}",
                        "role": "user"
                        }
                    ],
                    "images": image_list
