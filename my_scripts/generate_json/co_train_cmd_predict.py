...
[
    {
        "messages": [
        {
            "content": "Imagine you are a robot programmed for navigation tasks. You have given a video of historical obervations: <iamge> <image>. Please analyse your assigned task.",
            "role": "user"
        },
        {
            "content": "",
...
class Json_Templater:
    def __init__(self):
        ...
        self.template_history_str = f"serial of historical obervations:<history_image>. "
        
        
        self.template_question_str = f"Please analyse your assigned task."
    ...
    def fill_in_json_template(self):
        ...
        for folder in folder_list:
            ...
            for step in range(len(merged_actions)):
                ...
                json_assistant = {
                    "messages": [{
                        "content": f"My task is: {instruction_str}",
                        "role": "assistant"
                        }
                    ]
                }
                ...
                question_str = self.template_question_str
                ...
                if(step < self.max_historical_image_num):    
                    ...
                    history_str = self.template_history_str.replace("<history_image>",historical_image_str)
                    for i in range(step):
                        image_list.append(f"{self.prefix}/{episode_id}/{episode_id}_{merged_indices[step-i-1][0]}.jpg")
                    image_list.reverse()  # 这里我们是倒着放的，所以需要反转
                    image_list.append(f"{self.path_prefix}/{episode_id}/{episode_id}_{merged_indices[step][0]}.jpg")
                    print("step < self.max_historical_image_num",step,instruction_str)
                    ...
                    if self.uniform_sample_history:
                        start_to_current_indices = merged_indices[:step]
                        start_to_current_indices = self.uniform_sample(start_to_current_indices, self.max_historical_image_num)
                        for i in range(self.max_historical_image_num):
                ...
                json_user = {
                    "messages": [{
                        "content": f"{header_str}{history_str}{question_str}",
                        "role": "user"
                        }
                    ],
                    "images": image_list
        ...
        with open(f"co_train_{self.json_file_save_path}", "wb") as f:
            f.write(json_bytes)       
